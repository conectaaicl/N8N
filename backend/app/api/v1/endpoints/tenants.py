...
def get_public_info(
    ...
    """
    Get public branding info for a tenant based on the host.
    """
    host = request.headers.get("host", "")
    tenant = get_tenant_by_host(db, host)
    if not tenant:
        # Fallback for development if host doesn't match
...
def update_tenant_settings(
    ...
    """
    # For now, we'll take the first tenant until auth is fully wired
    tenant = db.query(Tenant).first()
    if not tenant:
        raise HTTPException(status_code=404, detail="Tenant not found")
...
def register_tenant(
    ...
    """
    Register a new tenant (signup).
    Creates the schema, initial tables, and the admin user.
    """
